...
- **Optional AI explanation, cached for offline reuse.** With an `ANTHROPIC_API_KEY` set, the same button also gets a one-time AI rewrite of the alert's own headline into plain language — fetched once per alert, then cached on-device for 5 days, so a second look (even offline) doesn't need a fresh call. Without a key, this part is skipped silently and the glossary still works.
...
3. **AI explanation (optional):** Vercel → Settings → Environment Variables → add `ANTHROPIC_API_KEY` from console.anthropic.com. Without it, "Explain this alert simply" still works via the built-in glossary, it just won't have the AI-rewritten version.
